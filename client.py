...
class ModernConferenceClient:
    ...
    def _get_avatar_color(self, addr):
        """Gets a consistent color for a user based on their address."""
        if not addr:
    ...
    def reaction_updater_loop(self):
        """
        Checks for expired reactions and triggers a redraw if needed.
        This animates emojis.
        """
        if not self.running:
            return
    ...
    def canvas_updater_loop(self):
        """
        The *only* function that calls redraw_canvas.
        Runs at a fixed ~30 FPS on the main GUI thread.
        """
        if not self.running:
            return
    ...
    def send_emoji_reaction(self, emoji):
        """Send emoji reaction and animate it"""
    ...
    def send_reaction_message(self, emoji):
        """Sends a dedicated 'reaction' message and triggers local effect"""
    ...
    def _draw_video_tile(self, user, x, y, cw, ch):
        """Helper function to draw a single video tile on the canvas."""
    ...
    def toggle_vbg_state(self):
        """FIX 3 (Modified): New function to handle VBG switch state change."""
        if not MEDIAPIPE_AVAILABLE:
            messagebox.showwarning("Feature Disabled",
    ...
    def _cleanup_share_socket(self):
        """Safely closes the screen sharing socket and resets state."""
    ...
    def _cleanup_view_socket(self):
        """Safely closes the screen viewing socket and resets state."""
        with self.screen_share_lock:
            self.viewing_screen = False
            self.screen_maximized = False
            self.screen_frame = None
    ...
    def _stop_screen_share(self):
        """Signals share loop to stop and updates GUI. All network tasks are offloaded."""
        with self.screen_share_lock:
    ...
    def _stop_share_network_tasks(self):
        """Runs on a background thread to handle network cleanup for stopping a share."""
        try:
